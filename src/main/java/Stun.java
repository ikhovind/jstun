...
public class Stun {
    ...
    while (running) {
      ...
      executor.execute(
          () -> {
            ...
            Response response =
                formulateResponse(
                    packet.getData(), packet.getAddress().getAddress(), packet.getPort());
            if (response != null) {
              //  Prepares the response as a package to be sent
              DatagramPacket send =
                  new DatagramPacket(response.getByteResponse(), response.getByteResponse().length);
              send.setAddress(packet.getAddress());
              send.setPort(packet.getPort());

              try {
                log.info(
                    "sent packet with address: "
                        + send.getSocketAddress()
                        + "\nwith source address: "
  ...
  private void handleTCPConnection(Socket socket) throws IOException {
    ...
    Response response =
        formulateResponse(buffer, socket.getInetAddress().getAddress(), socket.getPort());
    if (response != null) {
  ...
  // handles attributes, returns null if all are understood, array of not understood attributes if
  // not
