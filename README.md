...
The server is written in Java, and has the following functionality:
  * Attributes:
    * Mapped-Address
        * This gives backwards compatibility with RFC 3489
    * XOR-Mapped-Address
  ...
  * What exactly defines a malformed request?
    * When the server needs to determine whether a message is a valid STUN-request it checks the following, in order:
        1. Are the first two bits zero?
        2. Is the message a request and not a response?
        3. Is the message a binding method?
    ...
    * If these requirements are all met, the server then checks whether the message is a request or an indication. 
...
few and even though we are not using a thread per client this can become insufficient, even with worker threads.
...
without writing an entirely new client. So writing larger tests would be one of the first steps for continued development.


If the program was to for example implement verification methods such as usernames or passwords the server would have 
to test these. We would have to write a client that would be able to send a request with these attributes, so that 
we could verify that the response of the server was correct.

The attributes that are able to be implemented without implementing a credential mechanism would be Fingerprint,
Nonce, Software and Alternate Server.

The attributes we would be able to implement with a credential mechanism would be Username, Message-Integrity and Realm.

#### Weaknesses
The largest weakness that currently affects our server the most is the lack of being able to handle any attributes sent 
...
#### Log4j
Log4j is used to write the state of the program to both the command line and a file called all.txt
...
both with a UDP and TCP connection.

If you want to host an instance of the server yourself then you only have clone the repo, then run the maven project by doing the following:

1. If you don't have git, maven and java installed, please install them, your java installation needs to be of version 11 or higher
...
4. As we struggled to get our CD working, we attempted to set up an elastic beanstalk with automatic integration from our
...
5. After trying and failing to copy the repo into the VM with scp, we finally got it working with an rsync-template for 
...
the project into a jar. The project is then run by the systemd-configuration, which currently looks like this:
```` 
[Unit]
Description=App

[Service]
WorkingDirectory=/home/app/stun/target/
ExecStart=java -jar core-java-0.1.0-SNAPSHOT.jar 
user=app
Restart=always
Type=simple
